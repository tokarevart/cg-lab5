...
//struct FullRenderer : public Renderer {
...
//            (minmax_y[1] - minmax_y[0]) * viewport.width(),
...
//                    auto v0 = mesh.verts[edge[0].pos];
//                    auto v1 = mesh.verts[edge[1].pos];
//                    bnd_pts[j] = interpolate(v0, v1, ts[j]);
...
//                float depth = bnd_depths[0] + static_cast<float>(off) * delta_depth;
//                int idx = (ay - minmax_y[0]) * viewport.width() + xdrawrange[0];
